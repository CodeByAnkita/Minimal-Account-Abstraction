...
# install command
-forge init
-forge install  eth-infinitism/account-abstraction@0.7.0 --no-commit
-forge install  openzeppelin/openzeppelin-contrats@v5.0.2 --no-commit
-update foundry.tomal  - add remapping in that
-headers getters
-forge build - to test contract work or not
-anvil -in terminal then you get address and that address paste in SendPackedUSerOp.s.sol and kill anvil also  copy ANVIL_DEFAULT_KEY: privatekey and ANVIL_DEFAULT_ACCOUNT:  public key paste in helperconfig.s.sol
- forge test --debug testEntryPointCanExecutecommands -vvv  -use shift+g and reverted and this is best debug in foundry
 

 # for zksync
 - forge install cyfrin/foundry-era-contracts@0.0.3 --no-commit
 -  https://sepolia.explorer.zksync.io/   - with that you can see all the accounts info related deploy contract by using zksync
